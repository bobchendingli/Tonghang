package com.csb.utils;
/**
 * 
 * @author bobo
 *
 */
public class BundleArgsConstants {
	public static final String RESTURN_CODE_SUCC = "0000";
	public static final String RESTURN_CODE_FAIL = "0001";
	
    /**
     * new message broadcast arg
     */
    public static final String USERBEAN_EXTRA = "userbean_extra";
    public static final String MEETING_EXTRA = "meeting_extra";
    public static final String SURVEY_TYPE = "survey_type_extra";
    public static final String FILEBEAN_EXTRA = "filebean_extra";
    
    public static final int SURVEY_TYPE_START = 1;
    public static final int SURVEY_TYPE_END = 2;
    public static final int REQUEST_START = 0;
    public static final int REQUEST_SUCC = 1;
    public static final int REQUEST_FAIL = 2;
    public static final int REGISTER_SUCC = 3;
    public static final int UPDATESTATUS_SUCC = 4;
    public static final int UPLOAD_HEAD_PIC_SUCC = 5;
    public static final int UPLOAD_CODE_PIC_SUCC = 6;
    public static final int UPDATEQUESTIONNAIRE_SUCC = 7;
    public static final int IS_NOT_DOWNLOADFILE = 8;
}
